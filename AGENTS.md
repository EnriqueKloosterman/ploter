...
| Server | `cd server && pnpm test` | Run tests (vitest, 120 tests) |
...
| Client | `cd client && pnpm test` | Run tests (vitest, 37 tests) |
...
- **Server**: `server/src/controllers/export/` — PDF (pdfkit), DOCX (docx), EPUB (epub-gen), HTML, Fountain.
- **Routes**: `GET /api/export/:projectId/{html|pdf|docx|epub|fountain}` (authMiddleware).
- **Frontend**: `client/src/components/Canvas/ExportModal.tsx` — modal con 5 formatos, descarga directa.
...
- **Server tests** (`server/tests/`): 11 files, 120 tests. Covers all controllers (auth, project, snapshot, user, upload, export, AI) plus middleware (auth, validate) and utility functions (stripHtml, compileManuscript, buildProjectContext). Mongoose models, bcrypt, jsonwebtoken, OpenAI, pdfkit, docx, epub-gen are mocked.
- **Client tests** (`client/tests/`): 3 files, 37 tests. Covers apiFetch, sanitizeHtml, exportMarkdown.
