...
const ChapterPanel: React.FC = () => {
  const { project, addChapter, removeChapter, activeFocusChapterId, setActiveFocusChapterId } = useProject();
  ...
  const toggleChapter = (chapterId: string) => {
    setOpenChapters(prev => ({
      ...prev,
      [chapterId]: prev[chapterId] === false ? true : false // Por defecto asumimos que si no está, está Abierto. Hacemos toggle explicit. (o al revez)
    }));
  };
  ...
  return (
    <div className="bg-slate-900 border border-slate-700/50 rounded-xl overflow-hidden shadow-lg mb-6 max-h-[500px] flex flex-col">
              ...
              <div key={chapter.chapterId} className="group/item border border-slate-700/50 rounded-lg bg-slate-800 flex flex-col relative transition-all">
                ...
                >
                  ...
                  <div className="ml-auto mr-7 flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-slate-500 bg-slate-900 px-2 py-0.5 rounded-full border border-slate-700">
                    ...
                    {linkedNodes.length === 0 ? (
                      ...
                      linkedNodes.map((node) => (
                        <div key={node.id} className="relative flex items-start pl-4 py-1 group/beat">
                          ...
                          >
                            <p className="text-xs font-semibold text-blue-300 mb-1 leading-tight line-clamp-2">
                              {node.data.title || 'Escena sin título'}
                            </p>
