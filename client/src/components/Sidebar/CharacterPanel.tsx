  ...
  return (
    <div className="bg-slate-900 border border-slate-700/50 rounded-xl overflow-hidden shadow-lg mb-6 max-h-64 flex flex-col transition-all">
      ...
      {isPanelOpen && (
        <>
          ...
          <div className="p-3 overflow-y-auto space-y-2 grow relative group">
            {characters.length === 0 ? (
              <p className="text-xs text-slate-500 italic text-center py-4">Aun no hay personajes.</p>
            ) : (
              characters.map((char) => (
      ...
      <ConfirmModal
        ...
        message="Seguro que deseas purgar este personaje? Esta accion lo borrara de todos los nodos donde estaba vinculado."
